...
public class BitmapText extends Visual {
	...
	public static class Font extends TextureFilm {
		...
		spaceMeasuring:
			for (pos=0; pos <  width; pos++) {
				for (int j=0; j < height; j++) {
					int pixel = bitmap.getPixel(pos, j);
					Color c = new Color();
					c.set(pixel);
					if (pixel != color && c.a > 0f) {
		...
		public void dump(String name) {
			...
			// for (Map.Entry<Object, RectF> entry : frames.entrySet()) {
			// 	buffer.append("[" +entry.getKey() + "]:" + entry.getValue().width() + " ");
			// }
			// Log.d("Font", "name: " + name + " width: " + this.width() + " height: " + this.height() + " chars: " +  buffer.toString());
		}
	}
}
